...
class SafeCout {
  ...
  public:
    ...
    template <typename T> SafeCout &operator<<(T &&arg) {
 ...
 * 3. When you run the program now, the consumers should start running only after the data are
 * ready. Why do they run one by one, though? Check the CPU consumption with a tool like top in a
 * second shell. Ideally, we want 400% utilisation.
...
int main() {
    ...
    auto processData = [&](unsigned int threadIdx) {
        ...
        SafeCout{} << '[' << threadIdx << "] Data processing completed "
                   << (result ? "OK" : "with failure!") << '\n';
    };
