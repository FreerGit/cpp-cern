 ...
 * This weak pointers can be used to retreive the object pointed by the corresponding
 * `std::shared_ptr`, but it does not increase the reference count of the objects, and does not
 * prevent the deletion of the underlying objects if all shared pointers go out of scope. To *use*
 * the observed data, one has to create a `std::shared_ptr` from the `std::weak_ptr`, so that it is
 * guaranteed that the underlying object is alive.
 *
 ...
 *    In `setValue`and `getValue`, access the weak pointer, and use the data *only* if the memory is
 * still alive. Note: What you need is weak_ptr::lock(). Check out the documentation and the example
 * at the bottom: https://en.cppreference.com/w/cpp/memory/weak_ptr/lock
 *
 * --------------------------------------------------------------------------------------------
 */
